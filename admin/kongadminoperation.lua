...
-- auto register  server type 目前支持前三种，nacos 后面有空实现
...
function KongAdminOperation.create_service(cacheKey, name, tags, server_suffix)
    -- 1. 判断缓存中，是否存在service
    local cache_key = cacheKey .. ":service:" .. name
    ...
    res, err = admin_client(HTTP_METHODS.METHOD_POST, "/services", {
        name = name,
        tags = tags,
        host = name .. SERVER_SUFFIX_LIST[server_suffix] })
    parse_resp("service", res, err, cache_key)
end

--- create route by app name
---@param name string app name
function KongAdminOperation.create_route(cacheKey, name,tags)
    -- 1. 判断缓存中是否存在
    local cache_key = cacheKey .. ":route:" .. name
    if kong_cache:get(cache_key) then
        return "ok", nil
    end
...
function KongAdminOperation.create_upstream(cacheKey, name, tags, server_suffix, item)
    -- 判断缓存中 upstream 是否存在
    local cache_key = cacheKey .. ":upstream:" .. name
    if kong_cache:get(cache_key) then
        return "ok", nil
    end
...
function KongAdminOperation.get_targets(cacheKey, name, target_next)
    ...
    if res and res.status == 200 then
        ...
        for _, item in pairs(target_resp["data"]) do
            local kong_cache_key = cacheKey .. ":target:" .. name .. ":" .. item["target"]
        ...
        if ngx.null ~= target_resp["next"] then
            ...
            local next_targets = KongAdminOperation.get_targets(cacheKey, name, target_resp["next"])
...
function KongAdminOperation.delete_target(cacheKey, name, target, server_suffix)
    local cache_key = cacheKey .. ":target:" .. name .. ":" .. target
    LOG_INFO("[delete target]: upstream name :", name .. SERVER_SUFFIX_LIST[server_suffix], " ,target:", target)
...
function KongAdminOperation.put_target(cacheKey, name, target, weight, tags, server_suffix)
    -- get_targets use(fetch targets)
    local upsName = name .. SERVER_SUFFIX_LIST[server_suffix]
    local targets = KongAdminOperation.get_targets(cacheKey, name, "/upstreams/" .. upsName .. "/targets")
    if not targets then
    ...
    if weight ~= kong_weight then
        ...
        if kong_weight ~= 0 then
            KongAdminOperation.delete_target(cacheKey, name, target, server_suffix)
        end
...
function KongAdminOperation.kong_upstreams(upstream_next, server_suffix)
    ...
    if ngx.null ~= ups["next"] then
        local next_ups = KongAdminOperation.kong_upstreams(ups["next"])
...
function KongAdminOperation.getCurrentPlugin(pluginName)
    ...
    print("查询插件[select plugin]开始:" .. pluginName .. "cache_key 缓存key:" .. cache_key)
    if cache_key then
        --根据key 查询插件对象
        sync_server_plugin = plugins:select_by_cache_key(cache_key)
    end
    -- LOG_INFO("init worker,load sync_plugin:", pluginName, cjson.encode(sync_server_plugin))
